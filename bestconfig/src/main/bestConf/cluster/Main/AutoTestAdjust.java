...
import bestConf.cluster.Interface.*;
import bestConf.cluster.InterfaceImpl.SUTPerformance;
import bestConf.cluster.InterfaceImpl.SUTSystemOperation;
import bestConf.cluster.InterfaceImpl.SUTTest;
...
public class AutoTestAdjust implements ClusterManager{
	...
	public AutoTestAdjust(String configFilePath){
		...
		((SUTTest)sutTest).setShellofstartTest(startShell);
	...
	private void getProperties(){
		...
		csvResultPath = pps.getProperty("csvResultPath");
		startShell = pps.getProperty("startShell");
	}
...
//			ConfW_cluster.get(i).writetoConfigfile(hmTarget);
//			ConfW_cluster.get(i).uploadConfigFile();
//		}
		System.out.println(hmTarget.toString());
		double performance = new SocketPython().getResultFromPython(hmTarget);
//		long startTime = System.currentTimeMillis();
//		sutTest.startTest();
//		performance = System.currentTimeMillis() - startTime;
		systemPerformance.initial(performance,performance);
		return true;
	}
...
//				CSVWriter.writeOneTest(csvResultPath, hm, y);
