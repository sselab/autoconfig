...
public class BestConf {
    ...
    private static int timeOutInSeconds = 50;
...
//		manager.shutdown();
	...
	//initiate the attribute set, we only count those numeric properties
		//	 we initiate the range to be [(1-50%)*default,(1-50%)*default]
	//返回一个Instances集合，里面包含了初始值和各个配置的取值范围
	...
	public static void main(String[] args) {
		...
		if(args.length>0){
			...
			if(args.length>1){
				...
				if(args.length>2)
                    timeOutInSeconds = Integer.parseInt(args[2]);

			}
		}
		int finalMethod = method;
        String finalPreLoadDataPath = preLoadDataPath;
        new Thread(new Runnable() {
            @Override
            public void run() {
                startOneTest(finalMethod, finalPreLoadDataPath);
            }
        }).start();
